...
class HealthDataProcessor:
    ...
    def __init__(self):
        ...
        # Initialize with minimal data structure for immediate server startup
        self.create_sample_data_structure()
        self.create_realistic_sample_data()
    
    def load_data(self):
        """Load data from all available sources (async after server startup)"""
        try:
    ...
    def process_dengue_data_for_dashboard(self):
        ...
        try:
            ...
            # Note: Map data is now generated by generate_enhanced_map_data() method
            # which includes all major Pakistani cities with comprehensive disease data
    ...
    def generate_enhanced_map_data(self):
        ...
        try:
            ...
            logger.info("Starting enhanced map data generation")
            
            # Enhanced Pakistan cities with detailed information - Major and Critical Cities
            pakistan_cities = [
                ...
                # Additional Major Cities
                {
                    ...
                    'population': 2027001, 'climate_risk': 'medium', 'monsoon_affected': True
                },
                {
                    'name': 'Hyderabad', 'lat': 25.3960, 'lng': 68.3578, 'province': 'Sindh',
                    'population': 1734309, 'climate_risk': 'high', 'monsoon_affected': True
                },
                {
                    'name': 'Sialkot', 'lat': 32.4945, 'lng': 74.5229, 'province': 'Punjab',
                    'population': 655852, 'climate_risk': 'medium', 'monsoon_affected': True
                },
                {
                    'name': 'Bahawalpur', 'lat': 29.3956, 'lng': 71.6836, 'province': 'Punjab',
                    'population': 762111, 'climate_risk': 'medium', 'monsoon_affected': False
                },
                {
                    'name': 'Sargodha', 'lat': 32.0836, 'lng': 72.6711, 'province': 'Punjab',
                    'population': 659862, 'climate_risk': 'medium', 'monsoon_affected': True
                },
                {
                    'name': 'Sukkur', 'lat': 27.7052, 'lng': 68.8574, 'province': 'Sindh',
                    'population': 499900, 'climate_risk': 'high', 'monsoon_affected': True
                },
                {
                    'name': 'Larkana', 'lat': 27.5590, 'lng': 68.2123, 'province': 'Sindh',
                    'population': 490508, 'climate_risk': 'very_high', 'monsoon_affected': True
                },
                {
                    'name': 'Sheikhupura', 'lat': 31.7167, 'lng': 73.9667, 'province': 'Punjab',
                    'population': 473129, 'climate_risk': 'medium', 'monsoon_affected': True
                },
                {
                    'name': 'Jhang', 'lat': 31.2681, 'lng': 72.3317, 'province': 'Punjab',
                    'population': 414131, 'climate_risk': 'medium', 'monsoon_affected': True
                },
                {
                    'name': 'Dera Ghazi Khan', 'lat': 30.0561, 'lng': 70.6403, 'province': 'Punjab',
                    'population': 464742, 'climate_risk': 'high', 'monsoon_affected': False
                },
                {
                    'name': 'Gujrat', 'lat': 32.5740, 'lng': 74.0788, 'province': 'Punjab',
                    'population': 390533, 'climate_risk': 'medium', 'monsoon_affected': True
                },
                {
                    'name': 'Sahiwal', 'lat': 30.6682, 'lng': 73.1114, 'province': 'Punjab',
                    'population': 370426, 'climate_risk': 'medium', 'monsoon_affected': True
                },
                {
                    'name': 'Wah Cantonment', 'lat': 33.7969, 'lng': 72.7050, 'province': 'Punjab',
                    'population': 198891, 'climate_risk': 'high', 'monsoon_affected': True
                },
                {
                    'name': 'Mardan', 'lat': 34.1989, 'lng': 72.0402, 'province': 'KPK',
                    'population': 358604, 'climate_risk': 'low', 'monsoon_affected': False
                },
                {
                    'name': 'Kasur', 'lat': 31.1156, 'lng': 74.4502, 'province': 'Punjab',
                    'population': 314617, 'climate_risk': 'medium', 'monsoon_affected': True
                },
                {
                    'name': 'Mingora', 'lat': 34.7797, 'lng': 72.3614, 'province': 'KPK',
                    'population': 331091, 'climate_risk': 'low', 'monsoon_affected': False
                },
                {
                    'name': 'Rahim Yar Khan', 'lat': 28.4212, 'lng': 70.2989, 'province': 'Punjab',
                    'population': 420419, 'climate_risk': 'medium', 'monsoon_affected': False
                },
                {
                    'name': 'Okara', 'lat': 30.8081, 'lng': 73.4534, 'province': 'Punjab',
                    'population': 302168, 'climate_risk': 'medium', 'monsoon_affected': True
                },
                {
                    'name': 'Mirpur Khas', 'lat': 25.5276, 'lng': 69.0142, 'province': 'Sindh',
                    'population': 205913, 'climate_risk': 'high', 'monsoon_affected': True
                },
                {
                    'name': 'Chiniot', 'lat': 31.7200, 'lng': 72.9789, 'province': 'Punjab',
                    'population': 219254, 'climate_risk': 'medium', 'monsoon_affected': True
                },
                {
                    'name': 'Kamoke', 'lat': 31.9742, 'lng': 74.2247, 'province': 'Punjab',
                    'population': 199900, 'climate_risk': 'medium', 'monsoon_affected': True
                },
                {
                    'name': 'Mandi Bahauddin', 'lat': 32.5861, 'lng': 73.4917, 'province': 'Punjab',
                    'population': 165360, 'climate_risk': 'medium', 'monsoon_affected': True
                },
                {
                    'name': 'Jhelum', 'lat': 32.9425, 'lng': 73.7257, 'province': 'Punjab',
                    'population': 188803, 'climate_risk': 'medium', 'monsoon_affected': True
                },
                {
                    'name': 'Sadiqabad', 'lat': 28.3089, 'lng': 70.1261, 'province': 'Punjab',
                    'population': 141033, 'climate_risk': 'medium', 'monsoon_affected': False
                }
            ]
            ...
            logger.info(f"Enhanced map data generation completed. Generated data for {len(pakistan_cities)} cities")
    ...
    def generate_comprehensive_disease_trends(self):
        ...
        try:
            ...
            national_summary = self.current_data.get('national_summary', {})
            malaria_total = national_summary.get('malaria', 15740)  # Default from dashboard data
            dengue_total = national_summary.get('dengue', 1500)     # Default from dashboard data
            respiratory_total = national_summary.get('respiratory', 3628)  # Default from dashboard data
            ...
            # Store all trends - ensure this doesn't get overwritten
            if 'disease_trends' not in self.current_data:
                self.current_data['disease_trends'] = {}
            
            self.current_data['disease_trends'].update({
                'malaria': malaria_trends,
                'dengue': dengue_trends,
                'respiratory': respiratory_trends
            })
            
            logger.info(f"Generated comprehensive disease trends for 30 days: malaria={len(malaria_trends)}, dengue={len(dengue_trends)}, respiratory={len(respiratory_trends)} data points")
            
        except Exception as e:
            logger.error(f"Error generating comprehensive disease trends: {e}")
            # Fallback to ensure trends exist
            self.current_data['disease_trends'] = {
                'malaria': [],
                'dengue': [],
                'respiratory': []
    ...
    def get_map_data(self):
        """Get map data"""
        map_data = self.current_data.get('map_data', [])
        logger.info(f"get_map_data called: returning {len(map_data)} items")
        if len(map_data) == 0:
            logger.warning("Map data is empty! Current data keys: " + str(list(self.current_data.keys())))
        return map_data
